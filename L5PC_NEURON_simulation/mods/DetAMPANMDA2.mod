...
NET_RECEIVE (weight,weight_AMPA, weight_NMDA, Pv, Pr, u, tsyn (ms)){
	...
	:printf("NMDA weight = %g\n", weight_NMDA)
        ...
        if (Dep > 0) {
            ...
            A_AMPA = A_AMPA + weight_AMPA*factor_AMPA*Pr
            B_AMPA = B_AMPA + weight_AMPA*factor_AMPA*Pr
		    A_NMDA = A_NMDA + weight_NMDA*factor_NMDA*Pr
            B_NMDA = B_NMDA + weight_NMDA*factor_NMDA*Pr
